...
For comparison, take the cosine of the angle $\theta$
between vectors $i$ and $j$
in the euclidean space $\mathbb{R}^S$.

\[ D_{ij} = \cos(\theta) = \frac{ i \cdot j }{
...
Note also that $0 \leq D_{ij} \leq 1$
<!-- one should say non-negative but it's ugly -->
since $i$ and $j$ are always positive for all coordinates
and thus the angle between them can be at most $90^\circ$.

In euclidean geometry, the denominator of $H_{ij}$
can be seen as the mean of the square of the lengths
of $i$ and $j$.
However, I do not have a geometric intution regarding
the division of the dot product of $i$ and $j$
by the mean of the square of their lenghts.

The distance between two communities may also be computed
by obtaining first their probability distribution of surnames
and then using any metric on probability distributions,
eg Kantorovich or Kullback-Leibler.

<!-- After obtaining the distances between communities, -->
<!-- we would like to compute the dendrogram -->
...
TODO: check that Hedrick is indeed a metric between vectors.

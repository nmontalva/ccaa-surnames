...
trait <- GM_logit$G_logit               # Reemplazar por GM_df$M si analizas Multigeniture
names(trait) <- rownames(GM_logit)
...
ggplot(results, aes(x = height)) +
  ...
  labs(
    x = "Height",
    y = "Proportion / p-value (Kruskal-Wallis)"
  ) +
  theme_minimal()
...
results_long <- results %>%
  ...
  mutate(type = dplyr::recode(type, var_within = "Within", var_between = "Between")) %>%
  drop_na()

ggplot(results_long, aes(x = height, y = variance, fill = type)) +
  geom_area(alpha = 0.8) +
  scale_fill_manual(values = c("Within" = "blue", "Between" = "red")) +
       x = "Height", y = "Variance",
       fill = "variance") +
  theme_minimal()
...
ggplot(results, aes(x = height, y = var_ratio)) +
  ...
  labs(x = "Heigh", y = "Variance within / between") +
  theme_minimal()

...
STR_alelos_slash  <- read_delim("Datos/STR_ceros.csv",";", escape_double = FALSE, trim_ws = TRUE)
...
simple_Freq<-simpleFreq(mygen2) # Archivo de frecuencias simple
class(simple_Freq)
...
GST3<-calcPopDiff(simple_Freq,metric = "Gst") #Da un resultado similar a GST
GST4<-calcPopDiff(STR_freq,metric = "Gst") #Da un resultado demasiado dispar con los otros
GST5<-calcPopDiff(rrr,metric = "Gst")
...
#RST 1
RST <- calcPopDiff(simple_Freq,metric = "Rst", object = mygen)
diag(RST) <- NA
RST <- as.matrix (RST)
...
njRST<-nj(RST) #El ?rbol no es ultram?trico
lambda_value <- 0.5

...
z<-phylosig(consensus_tree,cv1,method = "lambda",test=T)
plot(z)
...
# Vectores de datos
trait_vectors <- list(sv1, gv1, av1, mv1,cv1)
...
row.names(Phy_sig) <- c("S", "G", "A", "M","Cluster")
...
# Vectores de datos
trait_vectors2 <- list(sv2, gv2, av2, mv2,cv2)
...
row.names(Phy_sig2) <- c("S", "G", "A", "M","Cluster")

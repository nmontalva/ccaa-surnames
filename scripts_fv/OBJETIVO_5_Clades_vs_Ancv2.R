...
print(result_df_S)
#result_df_S_f<-dplyr::filter(result_df_S,Node>15)
# Crear un order
#for (i in result_df_S_f) {
 # order <- list((length(unlist(Descendants(S_tree,i,type = "tips"))))) 
#  result_df_S <- do.call(rbind, order)
#}


# Crear el gráfico con líneas y puntos
# Expansión de result_df_S basado en paths, con distancia desde la raíz
...
ggplot(expanded_df, aes(x = Step, y = S_value, color = as.factor(Cluster), group = interaction(Cluster, Terminal_Node))) +
  geom_line(size = 1) +
...
ggplot(expanded_df, aes(x = Normalized_Step, y = S_value, color = as.factor(Cluster), group = interaction(Cluster, Terminal_Node))) +
  geom_line(size = 0.1, alpha = 0.6) +       # Línea de cada camino normalizado
  geom_point(size = 0.1) +                   # Puntos en cada nodo
  geom_smooth(aes(group = Cluster), method = "lm", se = FALSE, size = 1.2, linetype = "dashed") +  # Línea de tendencia
  labs(
    title = "Valores de S a lo largo del árbol",
    x = "Nodos desde la raíz",
    y = "Valor de S",
    color = "Cluster"
  ) +
  theme_minimal() +
...
clusters_table
# Tree
R_tree <- read.tree(text = "(((((((GUALLIGUAICA:0.128179,LA_POLVADA:0.128179)[&CI={0.871058,1.147971},ancstate={1.009515}]:0.13116,RINCONADA_DE_PUNITAQUI:0.259339)[&CI={0.854767,1.203206},ancstate={1.028986}]:0.148585,EL_DIVISADERO:0.407924)[&CI={0.738828,1.124912},ancstate={0.93187}]:0.057821,EL_ALTAR:0.465745)[&CI={0.700664,1.101748},ancstate={0.901206}]:0.17539,CANELILLA_OVALLE:0.641135)[&CI={0.528749,0.978733},ancstate={0.753741}]:0.040564,EL_ESPINAL:0.681698)[&CI={0.500054,0.963638},ancstate={0.731846}]:0.318302,(((((PUNITAQUI:0.042605,CANELA_BAJA:0.042605)[&CI={0.810824,0.977038},ancstate={0.893931}]:0.066623,CASTILLO_MAL_PASO:0.109227)[&CI={0.774211,1.028071},ancstate={0.901141}]:0.242948,MONTE_PATRIA:0.352175)[&CI={0.568442,0.970221},ancstate={0.769332}]:0.131883,(BARRAZA:0.264236,LA_CALERA:0.264236)[&CI={0.415893,0.800157},ancstate={0.608025}]:0.219821)[&CI={0.514279,0.941775},ancstate={0.728027}]:0.219526,MANQUEHUA:0.703583)[&CI={0.523303,1.034927},ancstate={0.779115}]:0.296417,HUENTELAUQUEN:1)[&CI={0.50012,1.053371},ancstate={0.776746}];")
#ancestral_nodes <- Ancestors(R_tree,1:15,"all")
paths
# Extraer los estados ancestrales
...
# Crear el gráfico con líneas y puntos
# Expansión de result_df_R basado en paths, con distancia desde la raíz
expanded_df <- do.call(rbind, lapply(1:length(paths), function(i) {
  path <- paths[[i]]
  ...
  # Normalizar el paso desde la raíz (0 a 1) para cada punto del camino
  normalized_steps <- seq(0, 1, length.out = length(path))
  data.frame(Normalized_Step = normalized_steps, Cluster = cluster, R_value = R_values, Terminal_Node = path[length(path)])
}))

# Graficar los valores de R a lo largo del camino de nodos desde la raíz para cada cluster
ggplot(expanded_df, aes(x = Normalized_Step, y = R_value, color = as.factor(Cluster), group = interaction(Cluster, Terminal_Node))) +
  geom_line(size = 0.1, alpha = 0.6) +       # Línea de cada camino normalizado
  geom_point(size = 0.1) +                   # Puntos en cada nodo
  geom_smooth(aes(group = Cluster), method = "lm", se = FALSE, size = 1.2, linetype = "dashed") +  # Línea de tendencia
  labs(
    title = "Valores de R a lo largo del árbol",
    x = "Nodos desde la raíz",
    y = "Valor de R",
    color = "Cluster"
  ) +
...
clusters_table
# Tree
A_tree <- read.tree(text = "(((((((GUALLIGUAICA:0.128179,LA_POLVADA:0.128179)[&CI={0.514536,0.581409},ancstate={0.547973}]:0.13116,RINCONADA_DE_PUNITAQUI:0.259339)[&CI={0.515488,0.599635},ancstate={0.557561}]:0.148585,EL_DIVISADERO:0.407924)[&CI={0.523321,0.616559},ancstate={0.56994}]:0.057821,EL_ALTAR:0.465745)[&CI={0.530646,0.627506},ancstate={0.579076}]:0.17539,CANELILLA_OVALLE:0.641135)[&CI={0.517552,0.626221},ancstate={0.571886}]:0.040564,EL_ESPINAL:0.681698)[&CI={0.513769,0.625723},ancstate={0.569746}]:0.318302,(((((PUNITAQUI:0.042605,CANELA_BAJA:0.042605)[&CI={0.557544,0.597684},ancstate={0.577614}]:0.066623,CASTILLO_MAL_PASO:0.109227)[&CI={0.536124,0.59743},ancstate={0.566777}]:0.242948,MONTE_PATRIA:0.352175)[&CI={0.493407,0.590435},ancstate={0.541921}]:0.131883,(BARRAZA:0.264236,LA_CALERA:0.264236)[&CI={0.504465,0.597264},ancstate={0.550864}]:0.219821)[&CI={0.494888,0.598126},ancstate={0.546507}]:0.219526,MANQUEHUA:0.703583)[&CI={0.488011,0.611567},ancstate={0.549789}]:0.296417,HUENTELAUQUEN:1)[&CI={0.48735,0.620958},ancstate={0.554154}];")
paths
# Extraer los estados ancestrales
...
# Crear el gráfico con líneas y puntos
# Expansión de result_df_A basado en paths, con distancia desde la raíz
expanded_df <- do.call(rbind, lapply(1:length(paths), function(i) {
  path <- paths[[i]]
  ...
  # Normalizar el paso desde la raíz (0 a 1) para cada punto del camino
  normalized_steps <- seq(0, 1, length.out = length(path))
  data.frame(Normalized_Step = normalized_steps, Cluster = cluster, A_value = A_values, Terminal_Node = path[length(path)])
}))

# Graficar los valores de A a lo largo del camino de nodos desde la raíz para cada cluster
ggplot(expanded_df, aes(x = Normalized_Step, y = A_value, color = as.factor(Cluster), group = interaction(Cluster, Terminal_Node))) +
  geom_line(size = 0.1, alpha = 0.6) +       # Línea de cada camino normalizado
  geom_point(size = 0.1) +                   # Puntos en cada nodo
  geom_smooth(aes(group = Cluster), method = "lm", se = FALSE, size = 1.2, linetype = "dashed") +  # Línea de tendencia
  labs(
    title = "Valores de A a lo largo del árbol",
    x = "Nodos desde la raíz",
    y = "Valor de A",
    color = "Cluster"
  ) +
...
clusters_table
# Tree
G_tree <- read.tree(text = "(((((((GUALLIGUAICA:0.128179,LA_POLVADA:0.128179)[&CI={-0.046512,0.145575},ancstate={0.049532}]:0.13116,RINCONADA_DE_PUNITAQUI:0.259339)[&CI={0.030048,0.271751},ancstate={0.150899}]:0.148585,EL_DIVISADERO:0.407924)[&CI={0.03501,0.302825},ancstate={0.168917}]:0.057821,EL_ALTAR:0.465745)[&CI={0.040635,0.318857},ancstate={0.179746}]:0.17539,CANELILLA_OVALLE:0.641135)[&CI={0.09487,0.407012},ancstate={0.250941}]:0.040564,EL_ESPINAL:0.681698)[&CI={0.091494,0.413069},ancstate={0.252282}]:0.318302,(((((PUNITAQUI:0.042605,CANELA_BAJA:0.042605)[&CI={0.105225,0.220523},ancstate={0.162874}]:0.066623,CASTILLO_MAL_PASO:0.109227)[&CI={0.120508,0.296603},ancstate={0.208556}]:0.242948,MONTE_PATRIA:0.352175)[&CI={0.136283,0.414986},ancstate={0.275634}]:0.131883,(BARRAZA:0.264236,LA_CALERA:0.264236)[&CI={0.25067,0.517224},ancstate={0.383947}]:0.219821)[&CI={0.155646,0.452188},ancstate={0.303917}]:0.219526,MANQUEHUA:0.703583)[&CI={0.093622,0.448522},ancstate={0.271072}]:0.296417,HUENTELAUQUEN:1)[&CI={0.057334,0.44111},ancstate={0.249222}];")
...
# Crear el gráfico con líneas y puntos
# Expansión de result_df_G basado en paths, con distancia desde la raíz
expanded_df <- do.call(rbind, lapply(1:length(paths), function(i) {
  path <- paths[[i]]
  ...
  # Normalizar el paso desde la raíz (0 a 1) para cada punto del camino
  normalized_steps <- seq(0, 1, length.out = length(path))
  data.frame(Normalized_Step = normalized_steps, Cluster = cluster, G_value = G_values, Terminal_Node = path[length(path)])
}))

# Graficar los valores de G a lo largo del camino de nodos desde la raíz para cada cluster
ggplot(expanded_df, aes(x = Normalized_Step, y = G_value, color = as.factor(Cluster), group = interaction(Cluster, Terminal_Node))) +
  geom_line(size = 0.1, alpha = 0.6) +       # Línea de cada camino normalizado
  geom_point(size = 0.1) +                   # Puntos en cada nodo
  geom_smooth(aes(group = Cluster), method = "lm", se = FALSE, size = 1.2, linetype = "dashed") +  # Línea de tendencia
  labs(
    title = "Valores de G a lo largo del árbol",
    x = "Nodos desde la raíz",
    y = "Valor de G",
    color = "Cluster"
  ) +
...
clusters_table
# Tree
M_tree <- read.tree(text = "(((((((GUALLIGUAICA:0.128179,LA_POLVADA:0.128179)[&CI={-0.103305,0.202191},ancstate={0.049443}]:0.13116,RINCONADA_DE_PUNITAQUI:0.259339)[&CI={-0.041574,0.342832},ancstate={0.150629}]:0.148585,EL_DIVISADERO:0.407924)[&CI={-0.007128,0.418807},ancstate={0.20584}]:0.057821,EL_ALTAR:0.465745)[&CI={0.012878,0.455363},ancstate={0.234121}]:0.17539,CANELILLA_OVALLE:0.641135)[&CI={0.148357,0.644789},ancstate={0.396573}]:0.040564,EL_ESPINAL:0.681698)[&CI={0.165674,0.677109},ancstate={0.421392}]:0.318302,(((((PUNITAQUI:0.042605,CANELA_BAJA:0.042605)[&CI={0.07256,0.255932},ancstate={0.164246}]:0.066623,CASTILLO_MAL_PASO:0.109227)[&CI={0.052299,0.332363},ancstate={0.192331}]:0.242948,MONTE_PATRIA:0.352175)[&CI={0.109608,0.552859},ancstate={0.331234}]:0.131883,(BARRAZA:0.264236,LA_CALERA:0.264236)[&CI={0.283215,0.707143},ancstate={0.495179}]:0.219821)[&CI={0.140948,0.61257},ancstate={0.376759}]:0.219526,MANQUEHUA:0.703583)[&CI={0.052061,0.616495},ancstate={0.334278}]:0.296417,HUENTELAUQUEN:1)[&CI={0.040801,0.651159},ancstate={0.34598}];")
#ancestral_nodes <- Ancestors(S_tree,1:15,"all")
paths
# Extraer los estados ancestrales
...
expanded_df <- do.call(rbind, lapply(1:length(paths), function(i) {
  ...
  # Normalizar el paso desde la raíz (0 a 1) para cada punto del camino
  normalized_steps <- seq(0, 1, length.out = length(path))
  data.frame(Normalized_Step = normalized_steps, Cluster = cluster, M_value = M_values, Terminal_Node = path[length(path)])
}))

# Graficar los valores de S a lo largo del camino de nodos desde la raíz para cada cluster
ggplot(expanded_df, aes(x = Normalized_Step, y = M_value, color = as.factor(Cluster), group = interaction(Cluster, Terminal_Node))) +
  geom_line(size = 0.1, alpha = 0.6) +       # Línea de cada camino normalizado
  geom_point(size = 0.1) +                   # Puntos en cada nodo
  geom_smooth(aes(group = Cluster), method = "lm", se = FALSE, size = 1.2, linetype = "dashed") +  # Línea de tendencia
  labs(
    title = "Valores de M a lo largo del árbol",
    x = "Nodos desde la raíz",
    y = "Valor de M",
    color = "Cluster"
  ) +
...
expanded_df <- do.call(rbind, lapply(1:length(paths), function(i) {
  ...
  data.frame(Normalized_Step = normalized_steps, 
             Cluster = cluster, 
             S_value = S_values, 
...
ggplot(expanded_df, aes(x = Normalized_Step, y = S_value, color = as.factor(Cluster), group = interaction(Cluster, Terminal_Node))) +
  ...
  theme_minimal() +
  scale_color_manual(values = c("blue", "green"))  # Colores para cada cluster (ajustar según los clusters)
dev.off()
...
# Crear el gráfico con líneas y puntos
# Expansión de result_df_M basado en paths, con distancia desde la raíz
expanded_df <- do.call(rbind, lapply(1:length(paths), function(i) {
  path <- paths[[i]]
  cluster <- result_df_M$Cluster[match(path[length(path)], result_df_M$Node)]
  M_values <- result_df_M$M_value[match(path, result_df_M$Node)]
...
#### si esto fueran series de tiempo.
ts_prueba <- as.ts(result_df_A)
str(ts_prueba) # Está mal conceptualizado. Hay trabajar en esto
plot(ts_prueba) 
acf(ts_prueba, type = "covariance")
